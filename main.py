...
bot_data = load_data()
PANEL_KEYS = [f"KEY-{random.randint(10000, 99999)}-{random.randint(10000, 99999)}" for _ in range(50)]
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_member(context.bot, user.id):
        ...
        if str(user.id) not in bot_data["users"]:
            bot_data["users"][str(user.id)] = {"name": user.first_name, "points": 0, "keys": [], "referrals": 0}
            save_data(bot_data)
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "check_join":
        if await is_member(context.bot, query.from_user.id):
            if str(query.from_user.id) not in bot_data["users"]:
                bot_data["users"][str(query.from_user.id)] = {"name": query.from_user.first_name, "points": 0, "keys": [], "referrals": 0}
                save_data(bot_data)
            ...
            await query.message.reply_text("❌ আপনি এখনো চ্যানেলে জয়েন করেননি!", reply_markup=get_join_menu())
